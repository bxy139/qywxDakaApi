...
#定义需要获取打卡人的ID listID
cat /daka/jiguan.csv  |cut -d , -f 2 >/daka/listID

#企业信息
corpid={企业ID}
appsecret=｛打卡应用secret｝
...
then
   ...
   #获取过去指定天打卡数据
    curl -s -X POST -H "Content-type:application/json" -d "{\"opencheckindatatype\":\"1\",\"starttime\":\"$kaishiTime\",\"endtime\":\"$jieshuTime\",\"useridlist\":$userlist}" https://qyapi.weixin.qq.com/cgi-bin/checkin/getcheckindata?access_token=$accesstoken | jq -r '.checkindata[] | [.userid, .checkin_time, .location_title, .location_detail, .exception_type]| @csv' >/daka/today.csv
#curl -s -X POST -H "Content-type:application/json" -d "{\"opencheckindatatype\":\"1\",\"starttime\":\"$kaishiTime\",\"endtime\":\"$jieshuTime\",\"useridlist\":$userlist}" https://qyapi.weixin.qq.com/cgi-bin/checkin/getcheckindata?access_token=$accesstoken >/daka/today.csv
	cat /daka/today.csv >>/tmp/for.tmp
   ...
   #格式化中文姓名
	...
	while  read name check_time check_location check_address exception_type;do
		xingming=$(cat /daka/jiguan.csv | grep $name |cut -d , -f 1|sed 's/\n//g')
...
cat /daka/today.csv >>/tmp/for.tmp
   ...
   #格式化中文姓名
	...
	#去除未打卡记录
	grep -v '未打卡' /daka/11.csv > /daka/12.csv
	...
	sed -i "1i 【$TodayTime园区员工进入打卡记录】" /daka/114.csv
	xiaoxi=$(cat /daka/114.csv | sed 's/,/-/g')
	iconv -f UTF-8 -t GBK /daka/114.csv -o /daka/$TodayTime机关打卡数据.csv

	#发送到机器人
	#curl -s 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=800000-5005-48003-0000050000000' \
   #-H 'Content-Type: application/json' \
   #-d "
   #{
    #    \"msgtype\": \"markdown\",
     #   \"markdown\": {
      #      \"content\": \"$TodayTime截止9:30分，今日到园区打卡员工$xixi人。详细数据请查收【远程办公】下发的汇总文档\"
       # }
   #}"
...
#发送数据到应用通知
	...
	filecsvname=$(curl -s -F media=@/daka/$TodayTime机关打卡数据.csv "https://qyapi.weixin.qq.com/cgi-bin/media/upload?access_token=$yuanchengaccesstoken&type=file" | jq '.media_id' | sed 's/"//g') 
